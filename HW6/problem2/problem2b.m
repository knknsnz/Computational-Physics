...
phi(1,:) =0;  phi(N,:) = 0;  phi(:,1) = phi0*ones(N,1);
phi(:,N) = 0; phi(1,1)=0; phi(N,1)=0;
    ...
    for i=2:(N-1)        % Loop over interior points only
     for j=2:(N-1)/2     
       newphi(i,j) = .25*(phi(i+1,j)+phi(i-1,j)+ ...
                               phi(i,j-1)+phi(i,j+1)+1/epsilon*h^2*rho);
       changeSum = changeSum + abs(1-phi(i,j)/newphi(i,j));
     end
    end
    for i=(N+1)/2+1:(N-1)        % Loop over interior points only
     for j=(N+1)/2+1:(N-1)        
       newphi(i,j) = .25*(phi(i+1,j)+phi(i-1,j)+ ...
                               phi(i,j-1)+phi(i,j+1)++1/epsilon*h^2*rho);
       changeSum = changeSum + abs(1-phi(i,j)/newphi(i,j));
...
figure(1); clf;
cLevels = 0:(0.05):1;    % Contour levels
